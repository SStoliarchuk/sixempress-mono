...
import { ReactFiber, TestChain, TestChainedItem, TestTools } from '../types/globals';
import { ThemeProvider, createMuiTheme } from '@material-ui/core';
import CssBaseline from '@material-ui/core/CssBaseline';
import { AmtsFieldProps } from '@sixempress/main-fe-lib';
...
((global as any).tc as TestChain) = {
	...
	focusBody() {
		for (const p of Array.from(document.querySelectorAll('[role="presentation"]'))) {
	...
	getAllMuiField(text: string | RegExp, container: RenderResult = screen as any) {
		...
		const filtered = labelNode.map((node: Node) => {
			...
			// the chips input field moves the input inside an addition div
			// so the position is not always the same, thus we querySelector it
			//
			// this is because before the input, the div chips are inserted
			const chipInput = (node.nextSibling?.childNodes[0] as Element)?.querySelector('input');
			if (chipInput) {
				return tc.wrap(chipInput);
			}
	...
	getByIcon(Icon: SvgIconComponent, container: HTMLElement = document.body) {
		const all = tc.getAllByIcon(Icon, container);
		if (all.length !== 1) {
			throw new Error("More than 1 icon found: " + (Icon as any).displayName);
		}
		return all[0];
	},
	
	getAllByIcon(Icon: SvgIconComponent, container: HTMLElement = document.body) {
		const generator = document.createElement('div');
		const root = ReactDOM.createRoot(generator);
		root.render("key" in Icon ? Icon as any : <Icon/>);
		
		let str = 'svg' + (generator.firstChild as HTMLElement).getAttributeNames()
			.map(k => `[${k}="${(generator.firstChild as HTMLElement).getAttribute(k)}"]`)
			.join("");
		
		const childs = Array.from((generator.firstChild as HTMLElement).children) as HTMLElement[];
		if (childs.length !== 0) {
			str += ' > ';
			str += childs.map(c => c.getAttributeNames().map(k => `[${k}="${c.getAttribute(k)}"]`).join("")).join(" + ");
		}
		
		// wrap svg not path child
		const res = Array.from(container.querySelectorAll(str)).map(c => tc.wrap(c.parentElement));
		if (res.length === 0) {
			throw new Error("Icon not found: " + (Icon as any).displayName);
		}
...
class TestChained implements TestChainedItem {
	...
	public type = (text: number | string | boolean, opts: {clear?: boolean} = {}) => {
		...
		userEvent.type(this.element, String(text));
	...
	public choseAmts = (model: object) => {
		// we click first as there could be some custom logic before the choseFn
		// ie in single product editor form, we set the target amts formcontrol in the state on click
		this.click();
	...
	public setDate = (n: number | Date | moment.Moment) => {
		let v: moment.Moment;
		if (typeof n === "number") {
			v = moment(n * 1000);
		} else if (isDate(n)) {
		...
		this.getReactInstance(18).getProps().onChange(v);

		return this;
	}

	public muiSelectMulti = (items: (number | string | RegExp)[]) => {
		this.executeMuiSelect(items, true);
