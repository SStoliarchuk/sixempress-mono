...
export class InternalOrdersPdf {

	// private static defaultTitle = 'Dettaglio Ordine'

	// public static async printOrder(id: string | InternalOrder) {
	// 	const dd = await InternalOrdersPdf.generateOrderPdf(typeof id === 'string' ? id : id._id);
	// 	PdfService.pdfAction(dd, 'print');
	// }

	// public static async generateOrderPdf(id: string): Promise<object> {
	// 	const item = await new InternalOrderController().fetchFullObject(id).toPromise();
	...
	// 				[{
	// 					text: {
	// 						text: (entrance.title || InternalOrdersPdf.defaultTitle) + '\n',
	// 						fontSize: 25,
	// 						alignment: 'center',
	// 						bold: true
	// 					},
