...
export class DataFormatterService {
	 ...
	 * 
	 * we escape < and > to &lt; and &gt; so it is safe to use inside setINnerHtml
	 * 
	 * we cant just yeet this inside <pre/> as with highlight it creates <span objects with classes to color the json
	 */
	static objToHtml(object: object, highlight?: boolean ): string {
		
		// transform to string
		const jsonString = this.replaceNewlineWithBrTag(
			// JSON.stringify(object, undefined, '&nbsp;') not working ??
