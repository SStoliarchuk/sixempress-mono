...
describe("ObjectUtils (object-utils.tsx)", () => {
	...
	describe('findLast()', () => {
		...
		it.only('works', () => {
			expect(ObjectUtils.findLast([1, 2, 3], (i) => i === 2)).toEqual(2);
			expect(ObjectUtils.findLast([1, 2, 3], (i) => i === 100)).toEqual(undefined);

			expect(ObjectUtils.findLast([{a:1}, {a: 2}, {b: 1}, {c: 2}], (i) => i.a === 2)).toEqual({a: 2});
			expect(ObjectUtils.findLast([{a:1}, {a: 2}, {b: 1}, {c: 2}], (i) => i.a === 200)).toEqual(undefined);
		});

	});

	describe('cloneDeepLogically', () => {

		it('works', () => {
			const a = {a: 1, b: {c: 2, d: 3, e: [1, 2, '4', {f: 4, e: [5, {g: {h: false}}]}]}};
			const cloned = JSON.parse(JSON.stringify(a));
	
			const logically = ObjectUtils.cloneDeep(a);
			
			expect(logically).toEqual(a);
			expect(logically).toEqual(cloned);
			expect(a).toEqual(cloned);
		});
		...
		it('circular', () => {
			...
			expect(ObjectUtils.cloneDeep(parent)).toEqual(parent);
		});

		it("doesnt alter original object", () => {
			const obj__ = {a: 1, b: "@", c: {d: 4, e: {f: 1}, s: [1, 2, {d: 3, g: [1]}]}};
			const saved = {a: 1, b: "@", c: {d: 4, e: {f: 1}, s: [1, 2, {d: 3, g: [1]}]}};
			expect(ObjectUtils.cloneDeep(obj__)).toEqual(saved);
			expect(obj__).toEqual(saved);
		});
